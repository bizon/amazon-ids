...
/*
 Kept separate from the `marketplaces` export below: the literal types are needed to
 derive the unions, but exposing them publicly would make `marketplace.domain` a type
 error for every entry that has no domain. The export is an alias, not a copy.
 */
const _marketplaces = [
  {
    code: 'be',
...
] as const satisfies readonly Marketplace[]

type MarketplaceEntry = (typeof _marketplaces)[number]

/**
 Marketplaces served by an Amazon storefront, as opposed to the multi-channel,
 invoicing and Amazon Pay ones which have no domain
 */
type DomainMarketplaceEntry = Extract<MarketplaceEntry, {domain: string}>

/**
 All the country codes of the marketplaces having a domain
 */
export type MarketplaceCode = DomainMarketplaceEntry['code']

/**
 All the available marketplace identifiers
 */
export type MarketplaceId = MarketplaceEntry['id']

/**
 All the available marketplace regions
 */
export type MarketplaceRegion = MarketplaceEntry['region']

/**
 All the available marketplace domains
 */
export type MarketplaceDomain = DomainMarketplaceEntry['domain']

/**
 All the available marketplaces
 */
export const marketplaces: readonly Marketplace[] = _marketplaces
